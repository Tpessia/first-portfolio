app.service("artistsService", function ($http, apiKeysService) {
    ...
    this.getTopAlbums = function (artist, page, limit) {
        return $http.get('//ws.audioscrobbler.com/2.0/?method=artist.gettopalbums&api_key=' + apiKeysService.getKey() +'&format=json&artist=' + encodeURIComponent(artist) + '&limit=' + encodeURIComponent(limit) + '&page=' + encodeURIComponent(page), { cache: true });
    }
});