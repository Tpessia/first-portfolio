app.service("topsService", function ($http, apiKeysService) {
    ...
    this.getTopArtists = function(page, limit) {
        return $http.get('//ws.audioscrobbler.com/2.0/?method=chart.gettopartists&api_key=' + apiKeysService.getLastFmKey() + '&format=json&limit=' + encodeURIComponent(limit) + '&page=' + encodeURIComponent(page));
    }
    ...
    this.getTopArtistsByTag = function(tag, page, limit) {
        return $http.get('//ws.audioscrobbler.com/2.0/?method=tag.gettopartists&api_key=' + apiKeysService.getLastFmKey() + '&format=json&limit=' + encodeURIComponent(limit) + '&tag=' + encodeURIComponent(tag) + '&page=' + encodeURIComponent(page));
    }
});