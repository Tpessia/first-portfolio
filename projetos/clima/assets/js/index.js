            ...
            success: function (json) {
                ...
                var init = {
                    all: function () {
                        ...
                        this.charts.buildLine("#chart-hourly", "hourly", [{ "label": "Temperatura", "data": "temp" }, { "label": "Umidade", "data": "hum" }, { "label": "Pressão (mBar - 1000)", "data": "pres" }]);

                        $("#hourly .t1").off().on("click", function () {
                            if (!$(this).find("a").hasClass("active")) {
                                init.charts.buildLine("#chart-hourly", "hourly", [{ "label": "Temperatura", "data": "temp" }, { "label": "Umidade", "data": "hum" }, { "label": "Pressão (mBar - 1000)", "data": "pres" }]);
                            }
                        });
                        ...
                        $("#hourly .t4").off().on("click", function () {
                            if (!$(this).find("a").hasClass("active")) {
                                init.charts.buildLine("#chart-hourly", "hourly", [{ "label": "Pressão (mBar - 1000)", "data": "pres" }], 2);
                            }
                        });

                        // SUMMARY

                        this.charts.buildBar("#chart-summary", "summary", [{ "label": "Temperatura", "data": "temp" }, ], 0, true);

                        $("#summary .t1").off().on("click", function () {
                            if (!$(this).find("a").hasClass("active")) {
                                init.charts.buildBar("#chart-summary", "summary", [{ "label": "Temperatura", "data": "temp" }], 0, true);
                            }
                        });

                        $("#summary .t2").off().on("click", function () {
                            if (!$(this).find("a").hasClass("active")) {
                                init.charts.buildLine("#chart-summary", "summary", [{ "label": "Pressão (Bar)", "data": "pres" }], 1, true);
                            }
                        });
                    },
                    ...
                    charts: {
                        buildLine: function (el, chart, content, index, anim) { // elem selector, charts[chart] content & chart parent id, {MyLabel, charts["chart"][data]}, index for right color, animate?
                            var chartData = [];
                            var dataLabel = [];
                        ...
                        buildBar: function (el, chart, content, index, anim) {
                            var chartData = [];
                            var dataLabel = [];
                            var labelColor = ["#3f51b5", "#7986cb", "#1a237e", "#c5cae9"];
