var pessia = {
    ...
    parallax: function(elems, mov) {
        if (typeof elems == "string") {
            $(elems).each(function () {
                var elem = this;

                var initialPos = $.isNumeric(bgPosition(mov, elem)) ? bgPosition(mov, elem) : (50 + mov);
                $(elem).css("background-position-y", initialPos + "%");

                $(window).scroll(function () {
                    var position = bgPosition(mov, elem);
            ...
            for (var key in elems) {
                if (typeof elems[key] == "number") {
                    parallax(key, elems[key]);
                } else {
                    throw "Invalid data type \"" + typeof elems[key] + "\". Expected \"string\"";
                }
            }
        } else {
            throw "Invalid data type \"" + typeof elems + "\". Expected \"object\"";
        }

        function scrollPercentage($selector) { // 0 a 100
            var wO = $(window).scrollTop(),
                wH = $(window).height(),
                sO = $selector.offset().top,
                sH = $selector.height();
            ...
            //console.log("wO ("+wO+") + wH ("+wH+") >= sO ("+sO+") && wH ("+wO+") <= sO ("+sO+") + sH ("+sH+")");

            if (wO + wH >= sO && wO <= sO + sH) {
                //console.log(((wO + wH) - sO) / delta);
                return ((wO + wH) - sO) / delta;
            }
        }

        function bgPosition(variation, selector) {
            return (Math.sqrt(Math.pow((scrollPercentage($(selector)) - 1), 2)) * (mov * 2) - mov) + 50;
            // sqrt pow -1 é para inverter 1,0.9,0.8 em 0,0.1,0.2
            // mov * 2 - mov é para 1 -> 0 virar 30 -> -30
            // + 50 é para referenciar o centro
        }
    }
}